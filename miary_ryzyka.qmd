...
bonds['log_return'] = np.log1p((100*bonds.iloc[:,0]).pct_change())
...
portfolio = san * weights[0] + bonds * weights[1] + pln_eur * weights[2]
df = pd.concat([san, bonds, pln_eur, portfolio], axis=1, join='inner')
df.columns = ['san', 'bonds', 'eur/pln', 'portfolio']

fig, axs = plt.subplots(2, 4, figsize=(20, 10))
axs[0, 0].plot(df.index, df.san, color='r')
axs[0, 0].set_title('san')
axs[0, 1].plot(df.index, df.bonds, color='r')
axs[0, 1].set_title('bonds')
...
# axs[1, 0].hist(df.san, bins=50)
axs[1, 0].set_title('san')
sns.ecdfplot(df.bonds, ax=axs[1, 1], color='r')
# axs[1, 1].hist(df.bonds, bins=50)
axs[1, 1].set_title('bonds')
sns.ecdfplot(df['eur/pln'], ax=axs[1, 2], color='r')
# axs[1, 2].hist(df['eur/pln'], bins=50)
axs[1, 2].set_title('eur/pln')
sns.ecdfplot(df.portfolio, ax=axs[1, 3], color='r')
# axs[1, 3].hist(df.portfolio, bins=50)
...
ax.scatter(df.san, df.bonds, df['eur/pln'], c='r', marker='o')
ax.set_xlabel('san')
ax.set_ylabel('bonds')
...
).set_properties(**{'margin-left': 'auto', 'margin-right': 'auto'})
```
...
```{python}
from scipy.stats import genextreme, gumbel_r, weibull_max, cramervonmises, kstest
...
# fit weibull
params_weibull = weibull_max.fit(maxs)
...
## Warunkowy rozkład przekroczenia {background-color="#f5f3f3"}
...
Dopasowanie parametrów dla uogólnionego rozkładu Pareto
...
fig.legend()
```
Test Craméra-von Misesa na zgodność rozkładu z uogólnionym rozkładem Pareto
```{python}
